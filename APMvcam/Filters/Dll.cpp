...
//  Directshow filter 'APM Virtual Cam'
...
STDAPI RegisterFilters( BOOL bRegister )
{
    ...
    if(bRegister)
    {
        hr = AMovieSetupRegisterServer(CLSID_VirtualCam, L"APM Virtual Cam", achFileName, L"Both", L"InprocServer32");
    }
    ...
    if( SUCCEEDED(hr) )
    {
        ...
        if( SUCCEEDED(hr) )
        {
            if(bRegister)
            {
                ...
                hr = fm->RegisterFilter(CLSID_VirtualCam, L"APM Virtual Cam", &pMoniker, &CLSID_VideoInputDeviceCategory, NULL, &rf2);
            }
            else
            {
...
//WCHAR model_path_hander[MAX_PATH];
//STDAPI DllInstall(BOOL bInstall, PCWSTR pszCmdLine)
//{
//    lstrcpyW(model_path_hander, pszCmdLine);
//    return RegisterFilters(bInstall);
//}
